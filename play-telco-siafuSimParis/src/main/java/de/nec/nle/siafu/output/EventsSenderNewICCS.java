...
public class EventsSenderNewICCS implements SimulatorOutputPrinter {
	...
	public EventsSenderNewICCS(final World world) {
		this.world = world;
		// Begin #1 PG 
		...
		if (lastEventsSendingAge > Constants.EVENTS_SENDING_FREQUENCY) {
			...
			for (Agent agent : world.getPeople()) {
				...
				if (agentName.startsWith("User")  ) {
					...
					if (traceOn) {
						System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
						System.out.println("Sent to WSN Endpoint   - Location Event : " + geolocationEvent.toString() );
						System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
					}
				...
				for (Overlay overlay : world.getOverlays().values()) {
					...
					if (overlayName.equals("CellularNetwork")) {
						...
						if (overlayValue.equals("Strong")) {
							...
							if (send2DSB){
								try {document = XMLHelper.createDocumentFromString(outNetworkEvent.toWSN());
								} catch (Exception e1) {}
							...
							if (traceOn) System.out.println( "outNetworkEvent Event : " + outNetworkEvent.toString());
						} else 
