...
public class EventsSenderRDF implements SimulatorOutputPrinter {
	...
	public EventsSenderRDF(final World world) {
		this.world = world;
		// Begin #1 PG 
		...
		if (lastEventsSendingAge > Constants.EVENTS_SENDING_FREQUENCY) {
			...
			for (Agent agent : world.getPeople()) {
				...
				if (agentName.equals("Traffic Infos Agent")) {
					for (Overlay overlay : world.getOverlays().values()) {
						...
						if (overlayName.equals("TrafficStatus")) {
							...
							if (send2DSB){
								...
								// TrafficJamEvent
								try {document = XMLHelper.createDocumentFromString(trafficJamEvent.toRDF("Siafu"));
								} catch (Exception e1) {}
							...
							if (traceOn)
								System.out.println( "Traffic   Event : " + trafficJamEvent.toRDF("Siafu") );
						}
					}
					...
					if (traceOn)
						System.out.println("Location Event : " + geolocationEvent.toRDF("Siafu") );
					...
					for (Overlay overlay : world.getOverlays().values()) {
						...
						if (overlayName.equals("Available")) {
							...
							if (!lastFirstTaxiAvailability.equals(overlayValue)) {
								...
								if (overlayValue.equals("False")) {
									...
									if (send2DSB){
										// send on the network - to WSN	
										// AvailabilityEvent
										try {document = XMLHelper.createDocumentFromString(availabilityEvent.toRDF("Siafu"));
										} catch (Exception e1) {}
									...
									if (traceOn)
										System.out.println( "AvailibilityEvent : " + availabilityEvent.toRDF("Siafu") );
								}
							}
							...
							if (!lastThirdTaxiPresence.equals(overlayValue)) {
								...
								if (overlayValue.equals("Off")) {
									...
									if (send2DSB){
										// send on the network - to WSN	
										// PresenceJamEvent
										try {document = XMLHelper.createDocumentFromString(presenceEvent.toRDF("Siafu"));
										} catch (Exception e1) {}
									...
									if (traceOn)
										System.out.println("Presence    Event : " + presenceEvent.toRDF("Siafu") );
								}
							}
							...
							if (!lastFirstTaxiUnexpectedEvent
								...
								if (overlayValue.equals("Yes")) {
									...
									if (send2DSB){
										// send on the network - to WSN	
										// PresenceJamEvent
										try {document = XMLHelper.createDocumentFromString(unexpectedEvent.toRDF("Siafu"));
										} catch (Exception e1) {}
									...
									if (traceOn)
										System.out.println("Unexpecte Event : " + unexpectedEvent.toRDF("Siafu"));
								}
							}
							...
							//									// send on the network - to WSN	
							//									// callEvent
							//									try {document = XMLHelper.createDocumentFromString(callEvent.toRDF());
							//									} catch (Exception e1) {}
	...
	public void sendCustomerAlert(String customerNumber, double[] coordinates) {
		...
		if (send2DSB){
			...
			try 
			{
				System.out.println("smsCustomerAlert.toRDF():\n");
				System.out.println("###############");
				System.out.println(smsCustomerAlert.toRDF("Siafu"));
				System.out.println("###############");
				document = XMLHelper.createDocumentFromString(smsCustomerAlert.toRDF("Siafu"));
			} catch (Exception e1)
			{e1.printStackTrace();
			}
		...
		if (traceOn)
			System.out.println( "SMSCusto Event : "+ smsCustomerAlert.toRDF("Siafu"));
	}
